...
— Rɪʏᴀɴ 🎀`;
  ...
  audio?.addEventListener("ended",()=>{if(musicBtn)musicBtn.textContent="▶";});
  // ---------- Arcade ----------
  const arcade = $("#arcade");
  let gameCleanup = () => {};
  ...
  // ---------- Advanced vibe picker ----------
  const vibeResult=$("#vibeResult");
  $$("[data-vibe]").forEach(btn=>btn.addEventListener("click",()=>{$$('[data-vibe]').forEach(x=>x.classList.remove('selected'));btn.classList.add('selected');if(vibeResult)vibeResult.textContent=btn.dataset.vibe+" mode activated. Birthday energy: MAXIMUM. 🥰";fireworks(12)}));
  // ---------- Birthday quiz ----------
  const quizBox=$("#quizBox");const questions=[["What is Aashu's birthday?",["16 September","10 October","25 December","1 January"],0],["What is this website made of?",["Homework","Best-friend chaos + love","Tax forms","Nothing"],1],["Who made this little universe?",["A random robot","Rɪʏᴀɴ 🎀","A movie studio","The moon"],1]];let qi=0,score=0;
  function showQuiz(){if(!quizBox)return;if(qi>=questions.length){quizBox.innerHTML=`<div class="quiz-question">🎉 ${score}/${questions.length} correct!</div><p class="soft">Certified birthday legend.</p>`;fireworks(25);return}const [q,opts,ans]=questions[qi];quizBox.innerHTML=`<div class="quiz-question">${q}</div><div class="quiz-options">${opts.map((o,i)=>`<button data-a="${i}">${o}</button>`).join('')}</div><div class="feature-result">Question ${qi+1} of ${questions.length}</div>`;$$('[data-a]',quizBox).forEach(b=>b.onclick=()=>{if(+b.dataset.a===ans)score++;b.textContent=+b.dataset.a===ans?'✓ Correct! ✨':'Not quite 😭';setTimeout(()=>{qi++;showQuiz()},600)})}showQuiz();
  // ---------- Candle wish ----------
  $("#blow")?.addEventListener("click",()=>{$("#cake")?.classList.add("blown");const r=$("#cakeResult");if(r)r.textContent="✨ Wish locked in! May this year be ridiculously good to you. 🎂💖";fireworks(35)});
  // ---------- Persistent wish wall ----------
  const wall=$("#wishWall"),input=$("#wishInput"),addWish=$("#addWish");let wishes=JSON.parse(localStorage.getItem("aashu-wishes")||"[]");function renderWishes(){if(wall)wall.innerHTML=wishes.map(w=>`<div class="wish-note">${String(w).replace(/[<>]/g,'')}<small>left in Aashu's universe ✦</small></div>`).join('')}renderWishes();addWish?.addEventListener("click",()=>{const v=input?.value.trim();if(!v)return;wishes.unshift(v);wishes=wishes.slice(0,12);localStorage.setItem("aashu-wishes",JSON.stringify(wishes));input.value='';renderWishes();fireworks(8)});input?.addEventListener("keydown",e=>{if(e.key==='Enter')addWish?.click()});
  // ---------- Secret sequence ----------
  const secretButtons=$$("#secretStars button"),secretResult=$("#secretResult");let secretStep=0;const order=[2,0,4,1,3];secretButtons.forEach((b,i)=>b.addEventListener("click",()=>{if(i===order[secretStep]){b.classList.add('done');secretStep++;if(secretStep===order.length){if(secretResult)secretResult.textContent="🔐 SECRET UNLOCKED: Aashu, you are officially one of the most important people in this tiny universe. 💖";fireworks(45)}}else{secretStep=0;secretButtons.forEach(x=>x.classList.remove('done'));if(secretResult)secretResult.textContent="Oops… the universe reset the sequence. Try again. 😭"}}));
  // ---------- Theme + fullscreen ----------
  const themes=['theme-pink','theme-blue','theme-gold'];let ti=0;$("#theme")?.addEventListener("click",()=>{document.body.classList.remove(...themes);ti=(ti+1)%themes.length;document.body.classList.add(themes[ti]);fireworks(8)});$("#fullscreen")?.addEventListener("click",async()=>{try{if(!document.fullscreenElement)await document.documentElement.requestFullscreen();else await document.exitFullscreen()}catch{}});
  // ---------- Keyboard easter egg ----------
  let keySeq='';document.addEventListener('keydown',e=>{keySeq=(keySeq+e.key.toLowerCase()).slice(-5);if(keySeq==='aashu'){fireworks(55);if(secretResult)secretResult.textContent='🌙 Hidden AASHU easter egg found. ✨'}});
